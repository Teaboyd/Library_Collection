export const ValidationCreateUser = (req,res,next) => {
	...
	if(!req.body.username){
		return res.status(401).json({message: "Please enter your username !" });
	}
	...
	if(req.body.username.length < 8 || req.body.username.length > 25){
		return res.status(401).json({message: "Please insert username between 8-25 letters!"})
	}

	if(req.body.password.length < 8 || req.body.password.length > 25){
		return res.status(401).json({message: "Please insert password between 8-25 letters!"})
	}
